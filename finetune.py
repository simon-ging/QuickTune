...
import numpy as np
from finetune_utils.utils import extend_metrics, prepare_model_for_finetuning, get_dataset_path,\
    get_icgen_dataset_info_json, get_number_of_classes, extend_metrics,\
    compute_gradient_norm
from finetune_utils.finetuning_stategies import SPRegularization, BatchSpectralShrinkage,\
    BehavioralRegularization, CoTuningLoss, Relationship,\
...
parser.add_argument('data_dir', metavar='DIR', default='datasets/inat', nargs='?',
...
group.add_argument('-vb', '--validation-batch-size', '--validation_batch_size', type=int,
...
group.add_argument('--opt-betas', '--opt_betas', default=None, type=float, nargs='+',
                   metavar='BETA',
...
group.add_argument('--decay-milestones', default=[90, 180, 270], type=int, nargs='+',
                   metavar="MILESTONES",
...
def main(args, args_text):
    ...
    if args.dataset_augmentation_path:
        try:
            ...
            assert args.dataset.startswith(
                "tfdsicgn"), "dataset_augmentation_path is only supported for tfds_icgn datasets"
    ...
    model, head_name = prepare_model_for_finetuning(model,
                                                    num_classes=args.num_classes,
    ...
    if compute_relationship:
        file_name = os.path.split(args.data_dir)[1] + "_"\
                    + args.model + "_"\
                    + args.dataset_augmentation_path.replace("/", "_") + "_"\
                    + args.dataset.replace("/", "_") + ".npy"
        output_dir = utils.get_outdir(os.path.join(args.output, "..",
                                                   "relationships") if args.output else './output/relationships')
        relationship = Relationship(loader_train, source_model, device,
    ...
    if utils.is_primary(args):
        ...
        initial_general_log = {"initial_eval_loss": eval_metrics['loss'],
    ...
    if utils.is_primary(args):
        final_general_log = {"max_memory_allocated": torch.cuda.max_memory_allocated(),
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            for backbone_regularization in backbone_regularizations:
                ...
                if isinstance(backbone_regularization, CoTuningLoss):
                    source_label = torch.from_numpy(relationship[target.cpu().numpy()]).to(
                        device).float()
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            backbone_grad_norm = (num_logs - 1) * backbone_grad_norm / num_logs + (
                    1 / num_logs) * temp_backbone_grad_norm
            head_grad_norm = (num_logs - 1) * head_grad_norm / num_logs + (
                    1 / num_logs) * temp_head_grad_norm
    ...
    return OrderedDict([('loss', losses_m.avg), ('head_grad_norm', head_grad_norm),
                        ('backbone_grad_norm', backbone_grad_norm)])
...
def make_one_batch_test(args, args_text, output_dir):
    args.test_mode = True
...
if __name__ == '__main__':
    ...
    decrease_batch_size = False
    report = None
    args, args_text = _parse_args()

    while out_of_memory_error:
        try:
            torch.cuda.empty_cache()
            if decrease_batch_size:
                utils.get_outdir(args.output if args.output else './output/train', args.experiment)
            ...
            if args.linear_probing:
                make_one_batch_test(args, args_text, output_dir)
            ...
            if isinstance(e, RuntimeError) and "out of memory" in str(e) and batch_size > 1:
                if output_dir and decrease_batch_size:
                    shutil.rmtree(output_dir)
            else:
                with open(os.path.join(current_dir, "experiments", "results.txt"), 'a+') as f:
                    f.write(f"xx {args.experiment} ({e})\n")
                # with open(f"experiments/results.txt", 'a+') as f:
                #    f.write(f"xx {args.experiment} ({e})\n")
                print(e)
                out_of_memory_error = False
            if has_synetune and args.report_synetune and report is not None:
                report(epoch=global_epoch + 1, eval_accuracy=0)
        if not decrease_batch_size:
            out_of_memory_error = False
