...
def param_groups_layer_decay(
    ...
    print(f"param groups for model: {type(model)}")
    show_param_groups_dict(param_groups, param_group_names)
...
def show_param_groups_dict(param_groups_dict, param_groups_names_dict):
    for group_name, group_content in natsorted(
        param_groups_dict.items(), key=lambda x: x[0]
    ):
        params = group_content["params"]
        param_names = param_groups_names_dict[group_name]["param_names"]
        wd = group_content["weight_decay"]
        lr = group_content["lr_scale"]
        print(f"{group_name:20s} {lr=:7.1e} {wd=:7.1e}")
        param_dict = {
            param_name: param for param_name, param in zip(param_names, params)
        }
        for param_name, param in natsorted(param_dict.items(), key=lambda x: x[0]):
            param_shape = tuple(param.shape)
            print(f"    {str(param_shape):20s} {param_name}")
        print()
